...
class InsTrialBalancePartners(models.TransientModel):
    ...
    def process_filters(self, data):
        ...
        if data.get('partner_ids',[]):
            filters['partners'] = self.env['res.partner'].browse(data.get('partners_ids',[])).mapped('vat')
        else:
            filters['partners'] = ''
    ...
    def process_data(self, data):
        if data:
            ...
            if data.get('account_ids',[]):
                where_clause += ' AND aa.id IN %s' % str(tuple(data.get('account_ids'))+tuple([0]))

            if data.get('partner_ids',[]):
                where_clause += ' AND aml.partner_id IN %s' % str(tuple(data.get('partner_ids'))+tuple([0]))
            ...
            for acc_part in account_partner:
                account_id = account_obj.browse(acc_part.get('account_id', False) or False)
                partner_id = partner_obj.browse(acc_part.get('partner_id', False) or False)
                ...
                lines.update({'%s-%s' %(account_id.code,partner_id.id):values})
            ...
            for key,vals in lines.items():
                ...
                partner = partner_obj.browse(vals.get('partner_id', False) or False)
    ...
    def get_filters(self, default_filters={}):
        ...
        partners = self.partner_ids if self.partner_ids else self.env['res.partner'].search(company_domain)
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            ...
            '.action_print_trial_balance_partner').with_context(landscape=True).report_action(
