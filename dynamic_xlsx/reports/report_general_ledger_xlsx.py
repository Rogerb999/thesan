...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.write_string(self.row_pos, 4, _('Entry Label'),
                                    self.format_header)
            col = 4
            if filter.get('tax_view',False):
                col += 1
                self.sheet.write_string(self.row_pos, col, _('Tax Base'),
                                        self.format_header)
            col += 1
            self.sheet.write_string(self.row_pos, col, _('Debit'),
                                    self.format_header)
            col += 1
            self.sheet.write_string(self.row_pos, col, _('Credit'),
                                    self.format_header)
            col += 1
        ...
        if acc_lines:
            for line in acc_lines:
                ...
                if filter.get('tax_view',False):
                    col += 1
                    self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('tax_base_amount')), self.line_header)
                col += 1
                self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('debit')), self.line_header)
                col += 1
                self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('credit')), self.line_header)
                col += 1
                self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('balance')), self.line_header)

                if filter.get('include_details', False):
                    account_id = acc_lines[line].get('id')
                    count, offset, sub_lines, tax_view = self.record.build_detailed_move_lines(offset=0, account=account_id,
                                                                                     fetch_range=1000000)
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            if filter.get('tax_view',False):
                                col += 1
                                self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('tax_base_amount')),
                                                        self.line_header_light_initial)
                            col += 1
                            self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('debit')),
                                                    self.line_header_light_initial)
                            col += 1
                            self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('credit')),
                                                    self.line_header_light_initial)
                            col += 1
                            self.sheet.write_number(self.row_pos, col, float(acc_lines[line].get('balance')),
                                                    self.line_header_light_initial)
                            ...
                            if filter.get('tax_view',False):
                                col+=1
                                self.sheet.write_number(self.row_pos, col,
                                                        float(sub_line.get('tax_base_amount')),self.line_header_light)
                            col+=1
                            self.sheet.write_number(self.row_pos, col,
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet_2.set_column(7, 7, 25)
